...
- **A gallery of 118 pictures ships with the integration**, 20 of them
  animated. Getting a picture onto a panel used to mean finding a file,
  putting it somewhere Home Assistant can read and typing its absolute path
  into an action. There is now a **Gallery** select entity on the device page:
  pick a motif, it appears. Categories are status, energy, weather, occasion,
  notification, decoration and symbol, weighted towards smart home -- doors,
  locks, leaks, presence, appliances, battery levels, grid flow, weather,
  bin day, doorbell.

- **`show_gallery_image`** action, for the same list from an automation. It
  takes the motif id, its plain name or the label the select entity shows, so
  an automation keeps working whichever of the three was written down.

- **`preload_gallery`** action, storing up to ten pictures in the panel's own
  slots. A stored picture is recalled with seven bytes instead of a whole
  frame buffer -- on a 32x32 panel, 7 rather than 12288. Ten is the panel's
  limit, not a chosen one: every write path stops there, while `show_slot`
  reads up to 255.

### Technical Details

- The artwork is generated by `tools/build_gallery.py` from rectangles, lines,
  circles and polygons rather than checked in as opaque files. Nothing is
  traced and no third-party image set is used, so the gallery carries this
  repository's licence. The same run writes the pictures, `index.json` and the
  action's dropdown, so the three cannot drift apart.
- Pictures are drawn for 32x32. Pixel art does not scale the way vector art
  does, and the supported panels come in thirty geometries, so
  `show_gallery_image` defaults to `fit` -- the whole picture scaled down and
  padded with black -- rather than cropping a narrow slice out of the middle
  of a wide panel.
- `gallery.py` reads the index once per Home Assistant run, in the executor.
