...
- [The bundled picture gallery](#the-bundled-picture-gallery)
- [Actions](#actions)
...
| **Sign** | Stop, no entry, give way, speed limit, crossing, no parking, roadworks, traffic light |
| **Office** | In a meeting, do not disturb, free, on air, headset, video call, muted microphone, desk |
...
Pictures shown often are worth storing on the panel itself. A stored picture is recalled with seven bytes instead of a whole frame buffer — on a 32×32 panel that is 7 bytes rather than 12288. The panel holds **ten** writable slots:
...
Afterwards `show_slot` with slot 1, 2 or 3 brings them straight back.

**Why 32×32.** Pixel art does not scale like vector art — a motif that reads at 32×32 turns to mush at 16 and goes blocky at 64. The supported panels come in thirty geometries, so no single bundled set fits them all. `show_gallery_image` therefore defaults to `fit`, which scales the whole picture down and pads with black, rather than cropping a narrow slice out of the middle on a wide panel.

Every picture is generated by `tools/build_gallery.py` from rectangles, lines, circles and polygons; the few that carry a word use a 3×5 face defined pixel by pixel rather than an imported font file. Nothing is traced and no third-party image set is used, so the whole gallery carries this repository's licence. To change one, edit its drawing function and re-run the script; it rewrites the pictures, `index.json` and the action's dropdown together.
